...
\section{Folgen und Konvergenz}
  ...
  \subsection{Konvergenz von Folgen}
    ...
    \subsubsection{Rechenregeln}
      ...
      \end{align*}
      Gilt $b_n\neq0\ (n\ge M)$, so ist die Folge $\left(\frac{a_n}{b_n}\right)^\infty_{n=m}$ definiert.
    \subsubsection{Wichtige Eigenschaften}
      \begin{enumerate}
        \item $a_n\rightarrow a\Longleftrightarrow\abs{a_n-a}\rightarrow0$
        \item Existiert ein $m\in\nat$ sodass für alle $n\in\nat$ mit $n\ge m$ gilt, dass $\abs{a_n-a}\le\alpha_n$ und gilt $\alpha_n\rightarrow0$, so konvergiert $(a_n)$ gegen $a$
...
\section{Unendliche Reihen}
  ...
  \subsection{Konvergenzkriterien für Reihen}
    ...
    \subsubsection{Absolute Konvergenz}
      ...
      Ist $\sumka$ absolut konvergent, so ist ist $\sumka$ konvergent. Insbesondere gilt die Dreiecksungleichung für Reihen
    ...
    Es sei $(a_k)$ eine Folge und $c_k:=\sqrt[k]{\abs{a_k}},k\in\nat$
      \begin{enumerate}
        \item Ist $(c_k)$ unbeschränkt, so ist $\sumka$ divergent
        \item Es sei $(c_k)$ beschränkt und $\alpha:=\limsup_\krai c_k$. Dann gilt:
          \begin{enumerate}[label=\roman*)]
    ...
    \subsubsection{Exponentialfunktion}
      ...
      $$\exp(x):=\sum^\infty_{k=0}\frac{x^k}{k!}=e^x$$
